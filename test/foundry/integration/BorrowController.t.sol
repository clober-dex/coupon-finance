...
contract BorrowControllerIntegrationTest is Test, CloberMarketSwapCallbackReceiver, ERC1155Holder {
    ...
    function _marketMake() internal {
        for (uint256 i = 0; i < wrappedCoupons.length; ++i) {
            ...
            CloberOrderBook(market).limitOrder(
                MARKET_MAKER, bidIndex, market.quoteToRaw(IERC20(key.asset).amount(50), false), 0, 3, ""
            );
    ...
    function testRepayOverCloberMarket() public {
        ...
        assertEq(couponManager.balanceOf(user, couponKeys[5].toId()), 57297729602, "COUPON0_BALANCE");
        assertEq(couponManager.balanceOf(user, couponKeys[6].toId()), 57297729602, "COUPON0_BALANCE");
    }
