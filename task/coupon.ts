...
import { BorrowController, CouponManager, DepositController, LeverageAdapter, RepayAdapter } from '../typechain'
...
task('coupon:create-clober-market')
  ...
  .setAction(async ({ asset, epoch }, hre) => {
    ...
    const repayAdapter = await getDeployedContract<RepayAdapter>('RepayAdapter')
    const leverageAdapter = await getDeployedContract<LeverageAdapter>('LeverageAdapter')
    const token = AAVE_SUBSTITUTES[chainId][asset]
    if (epoch < (await couponManager.currentEpoch())) {
      throw new Error('Cannot deploy for past epoch')
    }
    ...
    receipt = await waitForTx(repayAdapter.setCouponMarket({ asset: token, epoch }, deployedAddress))
    console.log(`Set repay adapter for ${asset}-${epoch} to ${deployedAddress} on tx ${receipt.transactionHash}`)

    receipt = await waitForTx(leverageAdapter.setCouponMarket({ asset: token, epoch }, deployedAddress))
    console.log(`Set leverage adapter for ${asset}-${epoch} to ${deployedAddress} on tx ${receipt.transactionHash}`)
  })
